...
const CourseCompletion = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            // Fetch all courses from Admin Backend
            const coursesRes = await adminApi.get("/courses");
            setCourses(coursesRes.data);

            // Fetch user's completed courses from User Backend
            const completedRes = await api.get("/users/completed-courses");
    ...
    const handleToggleCourse = async (courseId) => {
        const isCompleted = completedCourses.some(c => c._id === courseId || c.courseId === courseId);
        ...
        try {
            ...
            await api.post(
                `/users/complete-course/${courseId}`
            );
            await fetchData();
        } catch (error) {
    ...
    const isCourseCompleted = (courseId) => {
        return completedCourses.some(c => c._id === courseId || c.courseId === courseId);
    };
